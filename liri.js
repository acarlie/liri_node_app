...
const app = {
    ...
    this.readFilePromise(file)
      .then(function (res) {
        if (res.objs) {
          ...
          const values = async () => {
            const inquireRes = await that.inquireList(res.objs, message, name);
  ...
  liriHelp () {
    ...
    const helpResponse = async () => {
      const inquireRes = await that.inquireList(arr, 'About Liri & Liri Help', 'help');
  ...
  readFilePromise (file) {
    ...
    return new Promise((resolve, reject) => {
      fs.readFile(file, 'utf8', function (err, content) {
        const parsed = that.parseSaved(content);
        if (err) throw err;
        resolve(parsed);
      });
    });
